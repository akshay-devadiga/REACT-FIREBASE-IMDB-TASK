...
export const fetchoptionsmaker = snapshot => {
  let data = [];
  snapshot.forEach(childSnapshot => {
    console.log();
    data.push({
      label: childSnapshot.replace(/ *\([^)]*\) */g, ""),
      value: childSnapshot.match(/\(([^)]+)\)/)[1]
    });
  });
  return data;
};
...
class EditMovie extends React.Component {
      ...
      .then(snapshot => {
        ...
        for (var i = 0; i < data.length; i++) {
          if (data[i].id == this.props.match.params.id) {
            ...
            var castdata = "";
            if (data[i].cast) {
              const castlength = data[i].cast.length;
              castdata = fetchoptionsmaker(data[i].cast);
            }
            this.setState({
  ...
  submitmoviedetails(e) {
    ...
    console.log(this.props.match.params.id);
    var ref = firebase.database().ref(`movies/${this.props.match.params.id}`);
        ...
        <div id="moviecontainer">
          ...
          >
            ...
            {this.state.imagefileurl && (
              <img src={this.state.imagefileurl} width="60px" height="80px" />
            )}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <FileUploader
              accept="image/*"
              name="avatar"
            ...
            <Row>
              ...
              <Col xs="5">
                <ReactMultiSelectCheckboxes
                  ...
                  name="cast"
                  onChange={e => (
                    this.handleoptionsdata(e), this.updateState(e)
                  )}
                  value={this.state.cast}
                  width="70%"
                  onSelect={e => this.handleoptionsdata(e)}
                  required
                />
              </Col>
              <Col xs="1">
